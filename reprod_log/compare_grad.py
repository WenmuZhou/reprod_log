...
class Compare():
    def __call__(self,
        ...
        optim = paddle.optimizer.SGD(parameters=paddle_model.parameters(), learning_rate=lr)
...
if __name__ == "__main__":
    ...
    inputs = load_input('/Users/zhoujun20/Desktop/工作相关/VQA/unilm/grad_input.pth')

    optim = paddle.optimizer.AdamW(parameters=model.parameters(), learning_rate=5e-5, beta1=0.9, beta2=0.999,
                                   epsilon=1e-8,
                                   weight_decay=0.0)
    ...
    for i in range(10):
        ...
        for name, parms in model.named_parameters():
            if not parms.stop_gradient and parms.grad is not None:
                paddle_grad_dict[name] = {'grad_value': parms.grad, 'grad_requirs': parms.stop_gradient}
        torch_grad_dict = torch.load(f'/Users/zhoujun20/Desktop/工作相关/VQA/unilm/layoutlmft/grad/grad_{i}.pth')
        match_keys = ["query.weight", "key.weight", "value.weight", "dense.weight", "rel_pos_bias", "rel_pos_x_bias",
                      "rel_pos_y_bias", "visual_proj",
                      'ffnn_head.0.weight', 'ffnn_head.3.weight', 'ffnn_tail.0.weight', 'ffnn_tail.3.weight',
                      'rel_classifier.linear.weight']
